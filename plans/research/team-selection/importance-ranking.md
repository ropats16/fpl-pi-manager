...
| **Chip timing** | Schedule chips to the fixture calendar (WC before a swing → BB on the DGW → FH on the blank). Timing worth ~+49 pts/season; *choice* only ~3. Machinery to build now, bites from ~GW4+. | [tier-2/folklore] — FPL Copilot vendor study (method unpublished), not the top-50 page previously cited; "timing ≫ choice" direction corroborated by top-50 behaviour ([lint 2026-08-18](../gaffer-architecture/raw/season-long-machinery.md)) | [meta-and-timing](factors/meta-and-timing.md#chip-strategy--timing) |
