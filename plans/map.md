...
- Tree consolidated ([#3](https://github.com/ropats16/fpl-pi-manager/issues/3), 2026-08-16) — repo is the single authoritative tree: all four `.py` re-verified byte-identical (sha256) to `saf@fplpi.local:~/fpl/`, zero divergence. Killed the flattened chat-download cruft: `rm _index.md` (dead memory-index), fixed broken links in `fpl-agent-system.md` + `SKILL.md`. **PII decision: stay private, no scrub** — repo is private and the entry ID is functionally needed; revisit only if it goes public. Skill-file shape (single vs multi-agent, one-skill-per-task) deferred to the [#9](https://github.com/ropats16/fpl-pi-manager/issues/9) architecture grilling, not settled here.
